...
def flash_mla_sparse_merge_fwd(
    ...
    instead of recomputing QK GEMM.  For miss positions (hit_mask=0), computes
    QK^T via GEMM as usual.  Then performs online softmax + PV → output.

    Args:
        q: [s_q, h_q, d_qk], bfloat16
        kv: [s_kv, h_kv, d_qk], bfloat16  (h_kv=1 for MLA)
        actual_indices: [s_q, h_kv, topk], int32  (actual sparse indices)
        predicted_qk: [s_q, h_q, topk], float32
            Pre-computed QK scores = (raw_QK * sm_scale), already REMAPPED to
            actual_indices positions (i.e. predicted_qk[:, :, i] corresponds
            to actual_indices[:, :, i]).
    ...
    Returns:
        ...
        - lse: [s_q, h_q], float, 2-based log-sum-exp
    """
    results = flash_mla_cuda.sparse_merge_prefill_fwd(
