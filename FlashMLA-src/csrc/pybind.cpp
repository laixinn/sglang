...
// Sparse merge prefill: QK GEMM + merge hit positions + softmax + PV
std::vector<at::Tensor> sparse_merge_prefill_fwd(
    const at::Tensor &q,
    const at::Tensor &kv,
    const at::Tensor &indices,
    const at::Tensor &predicted_qk,
    ...
    auto dprops = at::cuda::getCurrentDeviceProperties();
    bool is_sm90 = dprops->major == 9;
    TORCH_CHECK(is_sm90, "Sparse Merge Prefill Kernel is only supported on SM90 architecture");

    CHECK_DEVICE(q);
    CHECK_DEVICE(kv);
    CHECK_DEVICE(indices);
    CHECK_DEVICE(predicted_qk);
    CHECK_DEVICE(hit_mask);
    ...
    TORCH_CHECK(predicted_qk.dtype() == torch::kFloat32, "predicted_qk must be float32");
    TORCH_CHECK(hit_mask.dtype() == torch::kInt32, "hit_mask must be int32");
    ...
    CHECK_SHAPE(kv, s_kv, h_kv, d_qk);
    CHECK_SHAPE(indices, s_q, h_kv, topk);
    CHECK_SHAPE(predicted_qk, s_q, h_q, topk);
    CHECK_SHAPE(hit_mask, s_q, h_kv, topk);

    TORCH_CHECK(q.stride(-1) == 1, "q must have contiguous last dim");
    TORCH_CHECK(kv.stride(-1) == 1, "kv must have contiguous last dim");
    TORCH_CHECK(indices.stride(-1) == 1, "indices must have contiguous last dim");
    TORCH_CHECK(predicted_qk.stride(-1) == 1, "predicted_qk must have contiguous last dim");
    TORCH_CHECK(hit_mask.stride(-1) == 1, "hit_mask must have contiguous last dim");
    ...
    float inv_sm_scale = (sm_scale != 0.0f) ? (1.0f / sm_scale) : 0.0f;

    SparseMergePrefillParams params = {
        s_q, s_kv, h_q, h_kv, d_qk, d_v, topk,
        sm_scale, sm_scale * 1.44269504f,  // sm_scale_div_log2 = sm_scale * log2(e)
        ...
        (float*)predicted_qk.data_ptr(),
        int64_stride_to_int(predicted_qk.stride(0)),
        int64_stride_to_int(predicted_qk.stride(1)),
        (int*)hit_mask.data_ptr(),
        int64_stride_to_int(hit_mask.stride(0)),
        inv_sm_scale,
    ...
    sm90::run_merge_fwd_kernel(params);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("sparse_merge_prefill_fwd", &sparse_merge_prefill_fwd);
}