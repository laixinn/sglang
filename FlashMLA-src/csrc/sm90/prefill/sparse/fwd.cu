...
// TMA params for QK-only kernel (no output TMA needed)
template<
    typename Shape_Q, typename TMA_Q
>
struct QKOnlyTmaParams {
    Shape_Q shape_Q; TMA_Q tma_Q;
};
...
struct MergeSharedMemoryPlan {
    ...
    bool is_kv_valid[2][B_TOPK];
    bool is_hit[2][B_TOPK];       // hit mask for merge
    float2 sM[32];
    float2 sL[64];
    float final_max_logits[64], final_lse[64];
    transac_bar_t bar_q, bar_k0_free[2], bar_k0_ready[2], bar_k1_free[2], bar_k1_ready[2], bar_is_kv_valid_ready;
...
template<typename TmaParams>
__global__ void __launch_bounds__(NUM_THREADS, 1, 1)
sparse_attn_merge_fwd_kernel(__grid_constant__ const SparseMergePrefillParams params, __grid_constant__ const TmaParams tma_params) {
#if IS_SM90
    const int q_h_idx = blockIdx.x % (params.h_q/B_H);
    const int s_q_idx = blockIdx.x / (params.h_q/B_H);
    const int warpgroup_idx = cutlass::canonical_warp_group_idx();
    ...
    extern __shared__ char wksp_buf[];
    MergeSharedMemoryPlan &plan = *reinterpret_cast<MergeSharedMemoryPlan*>(wksp_buf);
    Tensor sQ = make_tensor(make_smem_ptr(plan.q_o.q.data()), SmemLayoutQ{});
    Tensor sO = make_tensor(make_smem_ptr(plan.q_o.o.data()), SmemLayoutO{});
    Tensor sS0 = make_tensor(make_smem_ptr(plan.k[0].data()+64*512), SmemLayoutS{});
    Tensor sS1 = make_tensor(make_smem_ptr(plan.s.data()), SmemLayoutS{});
    ...
    if (warpgroup_idx == 0 || warpgroup_idx == 1) {
        ...
        auto merge_hit_qk = [&](auto warpgroup_idx, int topk_base) {
            ...
            float* gPredQK = params.predicted_qk +
                             s_q_idx * params.stride_predicted_qk_s_q +
                             q_h_idx * B_H * params.stride_predicted_qk_h_q +
                             topk_base;

            CUTE_UNROLL
            for (int row_idx = 0; row_idx < 2; ++row_idx) {
                int row = get_AorC_row_idx(row_idx, idx_in_warpgroup);
                CUTE_UNROLL
                for (int i = row_idx*2; i < size(rP); i += 4) {
                    int col = 8*(i/4) + (idx_in_warpgroup%4)*2;
        ...
        // Check if a topk block is entirely hit (Python sorts miss-first).
        // If hit_mask[block_start] == 1, all positions in the block are hit
        // (because miss positions are sorted before hit positions).
        auto is_block_all_hit = [&](int block_start) -> bool {
            return block_start < params.topk &&
                   __ldg(params.hit_mask + s_q_idx * params.stride_hit_mask_s_q + block_start) != 0;
        };
        ...
        auto store_O = [&]() {
            ...
            for (int i = 0; i < 2; ++i)
                scale_factors[i] = rL[i] == 0.0f ? 1.0f : 1.0f / rL[i];
            Tensor sO_local = make_tensor(make_smem_ptr(plan.q_o.o.data() + warpgroup_idx*B_H*(D_V/2)), SmemLayoutOTiles<4>{});
            bf16* stsm_addrs[4];
            int stsm_row = (idx_in_warpgroup/32)*16 + (idx_in_warpgroup%16);
            CUTE_UNROLL
            for (int i = 0; i < 64/16; ++i)
                stsm_addrs[i] = &sO_local(stsm_row, (idx_in_warpgroup%32/16*8) + 16*i);
            bool s2g_pred = warp_idx%4 == 0 && elect_one_sync();
            warpgroup_wait<0>();
            CUTE_UNROLL
            for (int tile_idx = 0; tile_idx < (D_V/2)/64; tile_idx += 1) {
                ...
                if (s2g_pred) {
                    int g_tile_idx = warpgroup_idx*4 + tile_idx;
                    SM90_TMA_STORE_3D::copy(&tma_params.tensor_map_O, plan.q_o.o.data() + g_tile_idx*(B_H*64), g_tile_idx*64, q_h_idx*B_H, s_q_idx);
                }
            }
            cute::tma_store_arrive();
        };
        ...
        if (warpgroup_idx == 0) {
            ...
            auto rescale_rO = [&](float scales[2]) {
                ...
                for (int row = 0; row < 2; ++row) {
                    CUTE_UNROLL
                    for (int i = row*2; i < size(rO); i += 4) { rO(i) *= scales[row]; rO(i+1) *= scales[row]; }
                    rL[row] *= scales[row];
                }
            };
            ...
            for (int block_idx = 0; block_idx < num_topk_blocks; block_idx += 2) {
                Tensor sV0l = make_tensor(make_smem_ptr(plan.k[0].data()), SmemLayoutKTilesTransposed<4>{});
                Tensor sV1l = make_tensor(make_smem_ptr(plan.k[1].data()), SmemLayoutKTilesTransposed<4>{});
                bool wg0_hit = is_block_all_hit(block_idx * B_TOPK);
                if (block_idx == 0) {
                    if (!wg0_hit) {
                        pipelined_wait_and_qkt_gemm_l();
                        pipelined_wait_and_qkt_gemm_r();
                        ...
                        // KV still needed for V — wait for producer to finish loading
                        plan.bar_k0_ready[0].wait(cur_bar_wait_phase);
                        plan.bar_k0_ready[1].wait(cur_bar_wait_phase);
                    }
                }
                // For hit-only blocks: load predicted_qk directly into rP (skip QK GEMM)
                if (wg0_hit) {
                    load_all_predicted_qk(Warpgroup0{}, block_idx * B_TOPK);
                }
                mask_rP(Warpgroup0{});
                // For miss/boundary blocks: merge hit positions after QK GEMM
                if (!wg0_hit) {
                    merge_hit_qk(Warpgroup0{}, block_idx * B_TOPK);
                }
                ...
                *(float2*)new_rM = plan.sM[idx_in_warpgroup/4];
                ...
                cur_bar_wait_phase ^= 1;
                if (block_idx+2 < num_topk_blocks) {
                    bool next_wg0_hit = is_block_all_hit((block_idx+2) * B_TOPK);
                    if (!next_wg0_hit) {
                        pipelined_wait_and_qkt_gemm_l();
                        warpgroup_wait<1>(); plan.bar_k1_free[0].arrive();
                        pipelined_wait_and_qkt_gemm_r();
                        warpgroup_wait<0>();
                    } else {
                        // Hit block: wait for KV (needed for V), skip QK GEMM
                        plan.bar_k0_ready[0].wait(cur_bar_wait_phase);
                        plan.bar_k0_ready[1].wait(cur_bar_wait_phase);
                        warpgroup_wait<0>(); // wait for current PV GEMM
                        plan.bar_k1_free[0].arrive();
                    }
                } else {
                    warpgroup_wait<0>(); plan.bar_k1_free[0].arrive();
                }
            }
            reduce_L(); 
            store_O();
            ...
            for (int block_idx = 0; block_idx < num_topk_blocks; block_idx += 2) {
                Tensor sV0r = make_tensor(make_smem_ptr(plan.k[0].data()+64*256), SmemLayoutKTilesTransposed<4>{});
                Tensor sV1r = make_tensor(make_smem_ptr(plan.k[1].data()+64*256), SmemLayoutKTilesTransposed<4>{});
                bool wg1_hit = is_block_all_hit((block_idx + 1) * B_TOPK);
                if (!wg1_hit) {
                    pipelined_wait_and_qkt_gemm();
                } else {
                    // Hit block: wait for KV (needed for V), skip QK GEMM
                ...
                warpgroup_wait<0>();
                // For hit-only blocks: load predicted_qk directly into rP
                if (wg1_hit) {
                    load_all_predicted_qk(Warpgroup1{}, (block_idx + 1) * B_TOPK);
                }
                mask_rP(Warpgroup1{});
                // For miss/boundary blocks: merge hit positions after QK GEMM
                if (!wg1_hit) {
                    merge_hit_qk(Warpgroup1{}, (block_idx + 1) * B_TOPK);
                }
                ...
                gemm_rs(false, TiledMMA_PV_LocalP{}, rS, sV1r, rO, idx_in_warpgroup);
                warpgroup_commit_batch();
                save_rS_to_sS(rS, sS1, idx_in_warpgroup);
                NamedBarrier::arrive_and_wait(256, NamedBarriers::wg0_s0_ready);
                gemm_ss(false, TiledMMA_PV_RemoteP{}, sS0, sV0r, rO, idx_in_warpgroup);
                warpgroup_commit_batch();
                fence_view_async_shared();
                NamedBarrier::arrive(256, NamedBarriers::wg1_s1_ready);
                warpgroup_wait<1>(); plan.bar_k1_free[1].arrive();
                warpgroup_wait<0>(); plan.bar_k0_free[1].arrive();
                cur_bar_wait_phase ^= 1;
            }
            reduce_L(); 
            store_O();
            if (idx_in_warpgroup%4 == 0) {
                for (int row = 0; row < 2; ++row) {
                    int real_row = get_AorC_row_idx(row, idx_in_warpgroup);
                    bool no_valid = rL[row] == 0.0f;
                    plan.final_max_logits[real_row] = no_valid ? -INFINITY : rM[row];
                    plan.final_lse[real_row] = no_valid ? -INFINITY : log2f(rL[row]) + rM[row];
                }
                fence_view_async_shared();
            }
            NamedBarrier::arrive_and_wait(128, NamedBarriers::warpgroup1_sync);
        ...
        // Producer warpgroup — loads KV tiles AND hit_mask
        cutlass::arch::warpgroup_reg_dealloc<72>();
        constexpr int GROUP_SIZE = 8, NUM_GROUPS = 128/GROUP_SIZE;
        constexpr int NUM_ROWS_PER_GROUP = B_TOPK / NUM_GROUPS;
        int idx_in_group = idx_in_warpgroup % GROUP_SIZE;
        ...
        int* gHitMask = params.hit_mask + s_q_idx*params.stride_hit_mask_s_q;
        ...
        int64_t token_indices[2][NUM_ROWS_PER_GROUP];
        bool is_token_valid[2][NUM_ROWS_PER_GROUP];
        bool is_token_hit[2][NUM_ROWS_PER_GROUP];
        ...
        auto load_token_indices = [&](int block_idx) {
            ...
            for (int buf_idx = 0; buf_idx < 2; ++buf_idx) {
                ...
                for (int local_row = 0; local_row < NUM_ROWS_PER_GROUP; ++local_row) {
                    ...
                    is_token_valid[buf_idx][local_row] = t >= 0 && t < params.s_kv;
                    is_token_hit[buf_idx][local_row] = (bool)__ldg(gHitMask + offs);
                }
            }
        };
        ...
        for (int block_idx = 0; block_idx < num_topk_blocks; block_idx += 2) {
            ...
            if (idx_in_group == 0) {
                ...
                for (int buf_idx = 0; buf_idx < 2; ++buf_idx)
                    ...
                    for (int local_row = 0; local_row < NUM_ROWS_PER_GROUP; ++local_row) {
                        plan.is_kv_valid[buf_idx][local_row*NUM_GROUPS+group_idx] = is_token_valid[buf_idx][local_row];
                        plan.is_hit[buf_idx][local_row*NUM_GROUPS+group_idx] = is_token_hit[buf_idx][local_row];
                    }
                plan.bar_is_kv_valid_ready.arrive();
            }
            cur_bar_wait_phase ^= 1;
...
void run_merge_fwd_kernel(const SparseMergePrefillParams& params) {
    ...
    auto tma_Q = cute::make_tma_copy(
        ...
        make_tensor(
            make_gmem_ptr((bf16*)params.q),
            make_layout(shape_Q, make_stride(params.stride_q_h_q, _1{}, params.stride_q_s_q))
        ),
        SmemLayoutQ{}
    );
    ...
    CUtensorMap tensor_map_O;
    {
        uint64_t sz[3] = {D_V, (unsigned long)params.h_q, (unsigned long)params.s_q};
        uint64_t st[2] = {D_V*sizeof(bf16), D_V*params.h_q*sizeof(bf16)};
        uint32_t bx[3] = {64, B_H, 1};
        uint32_t es[3] = {1, 1, 1};
        CUresult res = CUTLASS_CUDA_DRIVER_WRAPPER_CALL(cuTensorMapEncodeTiled)(
            &tensor_map_O, CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3,
            params.out, sz, st, bx, es,
            CUtensorMapInterleave::CU_TENSOR_MAP_INTERLEAVE_NONE,
            CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B,
            CUtensorMapL2promotion::CU_TENSOR_MAP_L2_PROMOTION_NONE,
            CUtensorMapFloatOOBfill::CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE
    ...
    auto kernel = &sparse_attn_merge_fwd_kernel<decltype(tma_params)>;

    constexpr size_t smem_size = sizeof(MergeSharedMemoryPlan);
    CHECK_CUDA(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
