...
// Sparse merge forward: compute QK for all positions, merge hit positions
// with pre-computed predicted_qk, then softmax + PV → output.
struct SparseMergePrefillParams {
    int s_q, s_kv, h_q, h_kv, d_qk, d_v, topk;
    float sm_scale, sm_scale_div_log2;
    ...
    cutlass::bfloat16_t* __restrict__ q;    // [s_q, h_q, d_qk]
    cutlass::bfloat16_t* __restrict__ kv;   // [s_kv, h_kv, d_qk]
    int* __restrict__ indices;               // [s_q, h_kv, topk]  actual indices
    ...
    // Merge inputs
    float* __restrict__ predicted_qk;        // [s_q, h_q, topk]  (= raw_QK * sm_scale)
    int stride_predicted_qk_s_q;             // h_q * topk
    int stride_predicted_qk_h_q;             // topk
    int* __restrict__ hit_mask;              // [s_q, h_kv, topk]  1=hit, 0=miss
    int stride_hit_mask_s_q;                 // h_kv * topk
    float inv_sm_scale;                      // 1.0f / sm_scale
