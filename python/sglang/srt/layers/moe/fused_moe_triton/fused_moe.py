...
def apply_token_ids(A, idx, topk):
    ...
    A = torch.concat([A, torch.zeros([1, A.shape[1]], dtype=A.dtype, device=A.device)], axis=0)
    A = A[idx]
...
def invoke_fused_moe_kernel(
    ...
    if _is_cuda:
        ...
        A = apply_token_ids(A, clipped_token_ids, top_k)
        A_scale = apply_token_ids(A_scale, clipped_token_ids, top_k)

        expert_ids = torch.repeat_interleave(expert_ids, config["BLOCK_SIZE_M"], dim=0)[:num_tokens_post_padded[0]]
        expert_ids[clipped_token_ids == invalid_ids] = -1
        ...
        deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_contiguous(
            (A, A_scale), (B, B_scale), C_flatten, expert_ids
        )
        
        C = C_flatten[expert_ids != -1].view(C.shape)
    else:
